...
void NewEffect::apply(const sf::RenderTexture& input, sf::RenderTarget& output)
{
...
void NewEffect::prepareTextures(sf::Vector2u size)
{
...
void NewEffect::filterBright(const sf::RenderTexture& input, sf::RenderTexture& output)
{
...
void NewEffect::blurMultipass(RenderTextureArray& renderTextures)
{
	sf::Vector2u textureSize = renderTextures[0].getSize();

	for (std::size_t count = 0; count < 2; ++count)
	{
		blur(renderTextures[0], renderTextures[1], sf::Vector2f(50.f, 1.f / textureSize.y));
		blur(renderTextures[1], renderTextures[0], sf::Vector2f(50.f / textureSize.x, 0.f));
	}
}

void NewEffect::blur(const sf::RenderTexture& input, sf::RenderTexture& output, sf::Vector2f offsetFactor)
{
...
void NewEffect::downsample(const sf::RenderTexture& input, sf::RenderTexture& output)
{
...
void NewEffect::add(const sf::RenderTexture& source, const sf::RenderTexture& bloom, sf::RenderTarget& output)
{
